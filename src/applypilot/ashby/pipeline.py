"""Ashby discovery pipeline: scan companies, fetch jobs via HTTP API, insert to DB."""
...
# ---------------------------------------------------------------------------
# Titles loader (shared with all explore modules)
# ---------------------------------------------------------------------------
...
def run_ashby_pipeline(
    ...
    """Discover Ashby jobs and insert them into the jobs table."""
    from applypilot.config import load_env
    from applypilot.ashby.detector import detect_ashby_companies
    from applypilot.ashby.search import search_company
    from applypilot.ashby.db import (
    ...
    console.print(
        f"\n[bold blue]AshbyPilot[/bold blue]  "
        f"titles={len(titles)}  limit={limit}  resume={resume}  dry_run={dry_run}"
    )

    # --- Detect & sync companies ---
    console.print("\n[dim]Scanning jobs table for Ashby companies...[/dim]")
    detected = detect_ashby_companies()
    ...
    for idx, company in enumerate(companies, 1):
        ...
        try:
            ...
            console.print(f"[yellow]Resume with: applypilot exploreashby {limit}[/yellow]")
            break
